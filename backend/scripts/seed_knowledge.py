"""
Seed the Aayu Printing knowledge base into ChromaDB.

Run once (or re-run to refresh) from the backend/ directory:
    python seed_knowledge.py

The document is stored with doc_id="aayu_knowledge_base" so re-running
...
KNOWLEDGE_FILE = Path(__file__).parent / "app" / "data" / "aayu_knowledge.txt"
DOC_ID = "aayu_knowledge_base"
...
def main():
    ...
    metadata = {
        "source": "aayu_knowledge_base.txt",
        "uploaded_by": "system",
        "doc_type": "knowledge_base",
    }
    stored = add_chunks(DOC_ID, chunks, metadata)
    print(f"  {stored} chunks stored in ChromaDB under doc_id='{DOC_ID}'.")
    print("Done. The chatbot can now answer questions about Aayu Printing Studio.")
